...
public:
	...
	void check_conviction(string password);
};

public_record::public_record(string password, int conviction, string name, int age)
			:police_record(password,conviction){
    this->name = name;
    this->age = age;
}
...
void public_record::check_conviction(string password){
	if (police_record::check_conviction(password) != -1){
        cout << "전과기록 " << police_record::check_conviction(password) << "건" << endl;
    } else {
