...
void createMap(map< int, int >& m) {
    ...
    /* for(int i = 0 ; i < 10; i++){
        m.insert(make_pair(rnd.getInt(),rnd.getInt()));
    } */
...
void lambda_2() {
    ...
    createVector(v);
    int sum = 0;
    auto lmd2 = [&sum](int& element){
        sum += element;
    };

    for_each(v.begin(),v.end(),lmd2);
    // TODO: 반드시 for_each() 문과 람다 함수를 이용하여 
    ...
    cout << "average: " << (double)sum / v.size() << endl; // 0.0 대신 실제 평균 값(실수)을 출력하라.
}
