...
public:
    ...
    int putItem(int weight){

        permitWeight -= weight;

        if(itemWeight > permitWeight-itemWeight){
...
istream& operator >>(istream& ins,Box& b){
...
ostream& operator << (ostream& stream, Box& b){
    ...
    stream << "Box available weight : " << b.permitWeight << endl;
...
int main() {
    ...
    cout << box;
    box.putItem(20);        // box 객체에 물품 중량 추가. 
                            // 이때 남은 허용용량을 체크하고 부족하면 오류메세지 출력
